...
[z,p,k] =  butter(FilterOrder,fc./(fs/2),ftype);        %Low-pass filter design
sos = zp2sos(z,p,k);

EMG_sig_2k_lowpass = filtfilt(sos,1,EMG_sig_2k);        %Apply low-pass filter to EMG signal

figure;
subplot(1,2,1)
plot(t,EMG_sig_2k)                                       %Plot original (unfiltered) signal
hold on
plot(t,EMG_sig_2k_lowpass,'-.')                          %Plot low-pass filtered signal
xlim([5 5.25])
ylim([-1.15 1.15])
...
[P_End,f2]=pwelch(EMG_End,hamming(L),noverlap,nfft,fs); %Calculate power spectral density at the end of EMG signal
