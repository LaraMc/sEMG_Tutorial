...
%"hann" and "nuttallwin"
[Pxx,f]=pwelch(EMG_sig_5s-mean(EMG_sig_5s),hamming(L),noverlap,nfft,fs);
subplot(1,3,2)
plot(f,Pxx)
xlim([0 500])
ylim([0 3.75*1e-4])
...
[Pxx2,f2]=pwelch(EMG_sig_5s-mean(EMG_sig_5s),hamming(L2),noverlap2,nfft2,fs);
subplot(1,3,3)
plot(f2,Pxx2)
xlim([0 500])
ylim([0 3.75*1e-4])
...
[z_b,p_b,k_b] =  butter(FilterOrder1,fc./(fs/2),ftype);
sos1 = zp2sos(z_b,p_b,k_b);

% Chebyshev Type I filter
passbandripple=0.5;
[z_c,p_c,k_c] = cheby1(FilterOrder2,passbandripple,fc./(fs/2),ftype);
sos2 = zp2sos(z_c,p_c,k_c);

FreqPts=1000;
[h1,f1] = freqz(sos1,FreqPts,fs);                       %Frequency response of filter
[h2,f2] = freqz(sos2,FreqPts,fs);
...
% Zero-Pole-Gain design, Butterworth filter
[z,p,k] =  butter(FilterOrder,fc./(fs/2),ftype);
sos = zp2sos(z,p,k);

EMG_sig_2k_lowpass = filtfilt(sos,1,EMG_sig_2k);        %Filter the signal
figure;
subplot(1,2,1)
plot(t,EMG_sig_2k)
hold on
...
[Pxx,f]=pwelch(EMG_sig_2k-mean(EMG_sig_2k),hamming(L),noverlap,nfft,fs);
subplot(1,2,2)
plot(f,Pxx)
hold on;
%Calculate the power spectral density of low-pass filtered signal using Welch's method
[Pxx_lowpass,f]=pwelch(EMG_sig_2k_lowpass-mean(EMG_sig_2k_lowpass),hamming(L),noverlap,nfft,fs);
plot(f,Pxx_lowpass,'-.')
legend({'Original Signal';'Low-Pass Filtered Signal'})
xlim([0 500])
xlabel('Frequency (Hz)')
...
clear z;clear p;clear k;clear sos;
% Zero-Pole-Gain design, Butterworth filter
[z,p,k] =  butter(FilterOrder,fc./(fs/2),ftype);
sos = zp2sos(z,p,k);

EMG_sig_2k_highpass = filtfilt(sos,1,EMG_sig_2k);
figure;
subplot(1,2,1)
plot(t,EMG_sig_2k)
hold on
...
[Pxx_highpass,f]=pwelch(EMG_sig_2k_highpass-mean(EMG_sig_2k_highpass),hamming(L),noverlap,nfft,fs);
plot(f,Pxx_highpass,'-.')
legend({'Original Signal';'High-Pass Filtered Signal'})
xlim([0 500])
xlabel('Frequency (Hz)')
...
EMG_sig_2k_bandpass = filtfilt(sos,1,EMG_sig_2k);
figure;
subplot(1,2,1)
plot(t,EMG_sig_2k)
hold on
...
[Pxx_bandpass,f]=pwelch(EMG_sig_2k_bandpass-mean(EMG_sig_2k_bandpass),hamming(L),noverlap,nfft,fs);
plot(f,Pxx_bandpass,'-.')
legend({'Original Signal';'Band-Pass Filtered Signal'})
xlim([0 500])
xlabel('Frequency (Hz)')
...
EMG_sig_2k_notch = filtfilt(sos,1,EMG_sig_2k_noise);
figure;
subplot(1,2,1)
...
[Pxx,f]=pwelch(EMG_sig_2k_noise-mean(EMG_sig_2k_noise),hamming(L),noverlap,nfft,fs);
subplot(1,2,2)
plot(f,Pxx)
hold on;
%Calculate the power spectral density of the notch filtered signal using Welch's method
[Pxx_notch,f]=pwelch(EMG_sig_2k_notch-mean(EMG_sig_2k_notch),hamming(L),noverlap,nfft,fs);
plot(f,Pxx_notch,'-.')
...
xlabel('Time (s)')
ylabel('Amplitude')
h.Position = h.Position + [0 -0.25 0 0];
h2.Position = h2.Position + [0 -0.4 0 0.3];
...
bp_sig = filtfilt(sos,1,EMG_Gait_sig_2k-mean(EMG_Gait_sig_2k));
...
tk_env = filtfilt(sos,1,abs(tk));                           %low-pass filtering to get the shape/outline of the teager kaiser signal
...
EMG_av = filtfilt(sos,1,absEMG);                             %low-pass filtering the rectified EMG to obtain a moving average
h2=plot(t,EMG_av);

%If the filter is not applied twice,i.e. using sosfilt, there will be a delay in the moving
%average window

EMG_av_delay = sosfilt(sos,absEMG);  
...
% Calculating the Mean Frequency using built-in Matlab function
% Values may differ slightly based on the method used to compute the power spectral density
Mean_freq_Start = meanfreq(EMG_Start,fs ) ;
Mean_freq_End = meanfreq(EMG_End,fs ) ;
...
%Calculate the power spectral density at the start of the signal using Welch's method
[P_Start,f1]=pwelch(EMG_Start,hamming(L),noverlap,nfft,fs);
...
[P_End,f2]=pwelch(EMG_End,hamming(L),noverlap,nfft,fs);
h3=plot(f2,P_End,'b');
...
P = bsxfun(@times,width,P_Start);

% find all elements within the specified range
idx = find(0<=f1 & f1<=fs/2);

% compute the total power within the range
pwr = sum(P_Start(idx,:));

% compute the central moment within the range
cen_moment = sum(bsxfun(@times,P_Start(idx,:),f1(idx))) ./ pwr;

Mean_freq_Start=cen_moment;
%Alternatively, use built-in Matlab function: Mean_freq_Start=meanfreq(EMG_Start,fs);
Mean_freq_End=meanfreq(EMG_End,fs);
...
%Alternatively, use built-in Matlab function: Median_freq_Start=medfreq(EMG_Start,fs);
%Values may differ slightly based on the method used to compute the power spectral density
Median_freq_Start = medfreq(EMG_Start,fs);
Median_freq_End = medfreq(EMG_End,fs);
...
[P_BB,f1]=pwelch(EMG_BB_crop,hamming(L),noverlap,nfft,fs);
[P_BR,f2]=pwelch(EMG_BR_crop,hamming(L),noverlap,nfft,fs);
...
overlap = 0.75;                                              %This is equal to 75% overlap
noverlap=round(overlap*nfft);                                %Overlap in samples
ovlp=0.75;                                                   %Calculate using windows that overlap by 75%

%Calculate the magnitude-squared coherence between signals recorded from BB and BR
[c, f] = mscohere(EMG_BB_crop,EMG_BR_crop,hamming(nfft),nfft*ovlp,nfft,fs);
